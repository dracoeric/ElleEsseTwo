...
/*   Updated: 2019/11/29 16:06:05 by erli             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ls_consume_arg(t_ls_data *data, char *pathname)
{
	...
	if ((info = ls_create_path_info(pathname, 0)) != 0)
	{
		if (info->stat.st_mode & S_IFDIR && !(data->options & LS_DIR_AS_FILE))
		{
...
void	ls_consume_arg_list(int len, char **argv, t_ls_data *data)
{
	...
	if (data == 0 || data->arg_file_list == 0)
		return ;
	i = 0;
	while (i < len)
		ls_consume_arg(data, argv[i++]);
	if (data->arg_file_list->n_path == 0)
		data->arg_file_list = 0;
}
